...
//Only for Admins
const HistoryModel = require('./models/history'); // Import your Mongoose model
        ...
        if (token) {
            ...
            jwt.verify(token, secretKey, async (err, decoded) => {
                if (err) {
                    ...
                    const uploadHistory = await HistoryModel.find().sort({ timestamp: -1 }); // Sort by timestamp in descending order
