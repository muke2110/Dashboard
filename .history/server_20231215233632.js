...
app.post('/UploadRecords', async(req, res) => {


    upload.single("file")(req, res, async function (err) {
        // Handle MulterError
        if (err instanceof multer.MulterError && err.code === 'LIMIT_FILE_SIZE') {
            let errors = [{ message: "File size is more than 15MB" }];
            return res.render('admin_Dashboard', { errors });
        }

        // Handle other errors
        if (err) {
            let errors = [{ message: "File upload failed. Please try again." }];
            return res.render('admin_Dashboard', { errors });
        }
