...
//for students open this page for raising issue
app.get('/report-issue', (req, res) => {
    res.render('report-issue');
});


app.get('/studentIssues', (req, res) => {
    res.render('studentIssues');
});


app.get('/admin/view-issues', (req, res) => {
    // Assuming you fetch issues from MongoDB and store them in the `issues` variable
    const issues = /* Fetch issues from MongoDB */
  
    // Render the view-issues.ejs template and pass the issues variable
    res.render('view-issues', { issues });
});
...
app.post('/submit-issue', (req, res) => {
    ...
    if (!title || !description || !certificateType) {
      res.status(400).json({ error: 'All fields are required' });
      return;
    }
  
    const newIssue = new issueForm({ title, description, certificateType });
