...
const {collection_admin, collection_student, issueForm, collection_history} = require('./mongodb');
...
const {saveFilePathsToDB} = require('./Modules/Multer')
...
app.post('/admin_Dashboard/uploadCertificates', upload.fields([{ name: 'pdfFileInput', maxCount: 1 }, { name: 'csvFileInput', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        console.log(req.files.pdfFileInput[0].path) //this is a path .pdf
        const workbook = xlsx.readFile(req.files.csvFileInput[0].path);
        console.log(req.files.csvFileInput[0].path) //this is path for .xlsx
        ...
        if(value){
            ...
            console.log("Something went wrong and did'nt updated in collection_history")
        }
