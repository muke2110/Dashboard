...
/*app.get('/student_Dashboard', async (req, res) => {
    try {
      ...
      const students = await collection_student.find({});
...
app.post('/users/login', async (req, res) => {
    ...
    if(req.body.role =='student'){
        ...
        const user = await collection_student.findOne({ "roll_number": name });
        ...
        /*try {
            // Fetch all students from the database
            const students = await collection_student.findOne({"roll_number":  });
            res.render('student_Dashboard', { students });
          } catch (error) {
            console.error(error);
            res.status(500).send('Internal Server Error');
