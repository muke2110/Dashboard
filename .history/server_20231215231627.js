...
app.post('/UploadRecords', upload.single("file"), async(req, res) => {
    ...
    if(fileSize > 1024 * 1024 *5){
        errors.push({ message: "File size is more than 5MB" });
        res.render('admin_Dashboard', { errors });
        return;
    }

    if (!req.file.mimetype === "image/jpeg" || !req.file.mimetype === "image/png" || !file.mimetype === "application/pdf"){
        errors.push({ message : "File is not supported"});
        res.render('admin_Dashboard', { errors });
        return;
    }
