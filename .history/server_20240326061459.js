...
const { overlayTextOnTemplate } = require('./Modules/textOverlay');
...
app.post('/test', upload.fields([{ name: 'pdfFileInput', maxCount: 1 }, { name: 'csvFileInput', maxCount: 1 }]), (req, res) => {

    const formData = req.body;

    // Accessing Name
    const nameX = formData.coordinates.Name.x;
    const nameY = formData.coordinates.Name.y;

    console.log('Name X:', nameX);
    console.log('Name Y:', nameY);

    // Accessing Event Name
    const eventNameX = formData.coordinates['Event Name'].x;
    const eventNameY = formData.coordinates['Event Name'].y;

    console.log('Event Name X:', eventNameX);
    console.log('Event Name Y:', eventNameY);

    // Accessing Date
    const dateX = formData.coordinates.Date.x;
    const dateY = formData.coordinates.Date.y;
...
app.post('/admin_Dashboard/uploadCertificates', upload.single('file'),upload.fields([{ name: 'pdfFileInput', maxCount: 1 }, { name: 'csvFileInput', maxCount: 1 }]), async (req, res) => {
    try {
        // Ensure files were uploaded
        if (!req.file || !req.files.pdfFileInput || !req.files.csvFileInput) {
            return res.status(400).send('Missing file(s) uploaded.');
        }
        console.log("Files",req.files)
        const templatePDFPath = req.files.pdfFileInput[0].path; // Path to the template PDF file
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            const { Name, 'Event Name': eventName, Date: certificateDate } = entry;
            
            const outputPDFPath = path.join(__dirname, 'Records', `${index + 1}_certificate.pdf`);

            // Overlay text on the template PDF
            overlayTextOnTemplate(templatePDFPath, outputPDFPath, Name, eventName, certificateDate, req.body.coordinates);
