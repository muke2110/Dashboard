...
app.post('/download-certificate', (req, res) => {
    try {
        ...
        // Construct the full path to the "Records" folder
        const recordsFolderPath = path.join(process.cwd(), 'Records');

        // You might need to implement your own logic to find the certificate file within the specified path
        // For example, searching for a file in the folder based on the provided certificatePath
        const certificateFiles = fs.readdirSync(path.join(recordsFolderPath, certificatePath));

        // Assuming you pick the first file found, adjust this logic based on your requirements
        const certificateFullPath = path.join(recordsFolderPath, certificatePath, certificateFiles[0]);
