...
const {sendEmailWithAttachment} = require('./ModulesbulkUploadHandler')
...
app.post('/admin_Dashboard/uploadCertificates', upload.single('file'), async (req, res) => {
    try {
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            const { name, email, 'event name': eventName } = entry;
            ...
            await send(email, pdfPath, name, eventName);
        }

        res.send('Certificates generated and sent successfully.');
    } catch (error) {
