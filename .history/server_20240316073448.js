    ...
    if (token) {
        jwt.verify(token, secretKey, async (err, decoded) => {
            ...
            try {
                let user;
                if (role === 'student') {
                    user = await collection_student.find({ "email": decodedEmail });
                    console.log(user)
                } else if (role === 'admin') {
                    user = await collection_admin.find({ "roll_number": decodedEmail });
                    console.log(user)
                } else {
                    return res.status(400).send('Invalid role');
                }
                ...
                if (!user.resetOTP) {
                    return res.status(400).send('User does not have resetOTP property');
                }
                const isOTPValid = user.resetOTP.includes(ot);
                if (!isOTPValid) {
                ...
                let updatedUser;
                if(role === 'student'){
                    updatedUser = await collection_student.findOne({ "email": decodedEmail });

                } else if(role === 'admin'){
                    updatedUser = await collection_admin.findOne({ "email": decodedEmail });
