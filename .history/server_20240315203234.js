...
app.post('/forgetPassword', async (req, res) => {
    ...
    try {
        ...
        if (role === 'student') {
            user = await student(email); // Assuming student() function searches by email
            console.log(user)
            if (!user) {
                return res.status(400).send('User not found');
            }
            // Update or insert OTP into the student document
            await collection_student.updateOne(
                { roll_number: `${email}` },
                { $push: { resetOTP: `${otp}` } }// Upsert option inserts a new document if no matching document found
            );
        } else if (role === 'admin') {
            user = await adminInfo(email); // Assuming adminInfo() function searches by email
            console.log(user)
            if (!user) {
                return res.status(400).send('User not found');
            }
            // Update or insert OTP into the admin document
            await collection_admin.updateOne(
                { roll_number : email },
                { $set: { resetOTP: otp } },
                { upsert: true } // Upsert option inserts a new document if no matching document found
            );
        } else {
