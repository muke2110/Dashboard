...
app.post('/admin_Dashboard/uploadCertificates', upload.fields([{ name: 'pdfFileInput', maxCount: 1 }, { name: 'csvFileInput', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            console.log(entry)
            const {'name' :Name, 'event name': eventName, 'roll number': roll_number, 'Date': certificateDate } = entry;
            // Overlay text on the template PDF
            const outputPDFPath = path.join(__dirname, 'Records', `${index + 1}_certificate.pdf`);
            const pdfBytes = await overlayTextOnTemplate(templatePDFBuffer, Name, eventName, certificateDate, req.body.coordinates);
            ...
            const existingStudent = await collection_student.findOne({ "roll_number": roll });
            if (!existingStudent) {
