        ...
        jwt.verify(token, secretKey, async (err, decoded) => {
            ...
            try {
                ...
                if (role === 'student') {
                    ...
                    const updateResult = await collection_admin.updateOne(updateQuery, { $set: { password: hashedPassword } });
                    console.log('Update result:', updateResult);
                }
                ...
                const updatedOTPArray = use.resetOTP.filter((otpValue) => otpValue !== otp);
