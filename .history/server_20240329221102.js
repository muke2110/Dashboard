...
app.post('/admin_Dashboard/uploadCertificates', upload.fields([{ name: 'pdfFileInput', maxCount: 1 }, { name: 'csvFileInput', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        console.log("Body: ", parseFloat(req.body.namePositionX));
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            const { pdfFileInput, csvFileInput, namePositionX, namePositionY, eventPositionX, eventPositionY, datePositionX, datePositionY } = req.body;

            // Process the PDF file and overlay text
            const pdfBytes = await overlayTextOnTemplate(pdfFileInput.data, 'Name', 'Event', 'Date', {
                namePositionX: parseFloat(namePositionX),
                namePositionY: parseFloat(namePositionY),
                eventPositionX: parseFloat(eventPositionX),
                eventPositionY: parseFloat(eventPositionY),
                datePositionX: parseFloat(datePositionX),
                datePositionY: parseFloat(datePositionY)
            });
