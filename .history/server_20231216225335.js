...
app.get('/view-certificate/:certificatePath/:certificateId', (req, res) => {
    try {
        const certificatePath = req.params.certificatePath;
        const certificateId = req.params.certificateId;

        // Validate and sanitize the parameters to prevent directory traversal
        if (!isValidCertificatePath(certificatePath) || !isValidCertificateId(certificateId)) {
            return res.status(400).send('Invalid certificate path or ID');
        }

        // Construct the full path to the certificate file
        const certificateFullPath = path.join(__dirname, 'Records', certificatePath, certificateId);

        // Serve the certificate file
        res.sendFile(certificateFullPath);
    } catch (error) {
        console.error(error);
...
// Function to validate the certificate path (add more validation logic if needed)
function isValidCertificatePath(certificatePath) {
    // Implement your validation logic here
    return typeof certificatePath === 'string';
}

// Function to validate the certificate ID (add more validation logic if needed)
