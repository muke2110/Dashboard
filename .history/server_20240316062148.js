...
app.post('/verifyAndResetPassword', async (req, res) => {
    ...
    const token = req.cookies['Token']; // Make sure the cookie name matches what you're sending from the client
        ...
        jwt.verify(token, secretKey, async (err, decoded) =>{
            ...
            try {
                ...
                if (user.role === 'student') {
                    await collection_student.updateOne({ email: decodedEmail }, { $set: { password: hashedPassword } });
                } else if (user.role === 'admin') {
                    await collection_admin.updateOne({ email: decodedEmail }, { $set: { password: hashedPassword } });
                }
                ...
                if (role === 'student') {
                    await collection_student.updateOne({ email: decodedEmail }, { $set: { resetOTP: updatedOTPArray } }).th;
                } else if (role === 'admin') {
                    await collection_admin.updateOne({ email: decodedEmail }, { $set: { resetOTP: updatedOTPArray } });
                }
        
                // Password updated successfully
                res.clearCookie('Token'); // Corrected syntax for clearing the cookie
                res.send('Updated');
            } catch (error) {
                console.error(error);
                res.status(500).send('Server Error');
