...
app.get('/verificationPage',(req,res)=>{
    ...
    res.render('verificationPage', { token }); // Pass the token to the template
})
...
app.post('/forgetPassword', async (req, res) => {
    ...
    try {
        ...
        concatenatedData = email + otp
        const hashedData = await bcrypt.hash(concatenatedData, 10);

        // Redirect to verification page
        res.redirect(`/verificationPage?token=${hashedData}`);
...
app.post('/verifyAndResetPassword', async (req, res) => {
    const { email, otp, password, confirmPassword, token } = req.body;
    console.log(token)
    function decrypt(token, key) {
        const decipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(key), Buffer.from('16BYTEINITIALIZA'));
        let decrypted = decipher.update(Buffer.from(text, 'hex'));
        decrypted = Buffer.concat([decrypted, decipher.final()]);
        return decrypted.toString();
    }
    const [email, otp] = decrypt(token,10)
    try {
        let user;
        ...
        if (user.role === 'student') {
            await collection_student.updateOne({ email }, { $set: { password: hashedPassword } });
        } else if (user.role === 'admin') {
            await collection_admin.updateOne({ email }, { $set: { password: hashedPassword } });
        }

        // Remove OTP from array
        const updatedOTPArray = user.resetOTP.filter((otpValue) => otpValue !== otp);

        // Update user document with the modified OTP array
        if (user.role === 'student') {
            await collection_student.updateOne({ email }, { $set: { resetOTP: updatedOTPArray } });
        } else if (user.role === 'admin') {
            await collection_admin.updateOne({ email }, { $set: { resetOTP: updatedOTPArray } });
        }
