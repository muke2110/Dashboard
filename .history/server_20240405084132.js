...
app.post('/admin_Dashboard/uploadCertificates', upload.fields([{ name: 'pdfFileInput', maxCount: 1 }, { name: 'csvFileInput', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        const pdf_path = req.files.pdfFileInput[0].path
        const xlsx_path = req.files.csvFileInput[0].path
        /
        //updating into the collection_hsistor
        await collection_history.create({
            Pdf_path: pdfPath,
            xlsx_path: xlsxPath,
            timestamp: new Date()
        });
