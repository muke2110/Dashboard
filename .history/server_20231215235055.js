...
app.post('/UploadRecords', upload.single("file"), async (req, res, next) => {
    const roll_number = req.body.roll_number;
    let errors = [];

    // Handle MulterError
    if (req.file instanceof multer.MulterError && req.file.code === 'LIMIT_FILE_SIZE') {
        errors.push({ message: "Limit Size Exceeded" });
    ...
    const path = req.file.path; // Move this line here
