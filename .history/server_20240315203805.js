...
app.post('/forgetPassword', async (req, res) => {
    ...
    try {
        ...
        if (role === 'student') {
            user = await student(roll_number:email`,res); // Assuming student() function searches by email
            console.log(user)
            if (!user) {
            ...
            user = await adminInfo(email,res); // Assuming adminInfo() function searches by email
            console.log(user)
            if (!user) {
                return res.status(400).send('User not found');
            }
