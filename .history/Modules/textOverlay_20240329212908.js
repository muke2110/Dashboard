...
async function overlayTextOnTemplate(templatePDFBuffer, name, eventName, certificateDate, coordinates) {
    try {
        ...
        const newPageCanvas = await pdfDoc.embedPdf(canvasDoc);

        newPage.drawPage(newPageCanvas.page, {x: 0, y: 0, width: newPage.getSize().width, height: newPage.getSize().height});
