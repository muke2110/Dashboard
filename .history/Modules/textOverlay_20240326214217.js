...
async function overlayTextOnTemplate(templatePDFBuffer, name, eventName, certificateDate, coordinates) {
    try {
        ...
        // Copy pages from the overlay PDF to the template PDF
        const [overlayPage] = await pdfDoc.copyPages(pdfDocOverlay, [0]);
        
        // Insert the copied page to the template PDF
        pdfDoc.insertPage(0, overlayPage);
