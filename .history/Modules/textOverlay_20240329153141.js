...
async function overlayTextOnTemplate(templatePDFBuffer, name, eventName, certificateDate, coordinates) {
    try {
        ...
        // Access coordinates using exact keys from the coordinates object
        const xCoordinateName = parseFloat(coordinates.xCoordinateName);
        const yCoordinateName = parseFloat(coordinates.yCoordinateName);
        const xCoordinateEventName = parseFloat(coordinates.xCoordinateEventName);
        const yCoordinateEventName = parseFloat(coordinates.yCoordinateEventName);
        const xCoordinateDate = parseFloat(coordinates.xCoordinateDate);
        const yCoordinateDate = parseFloat(coordinates.yCoordinateDate);
