...
async function overlayTextOnTemplate(templatePDFBuffer, name, eventName, certificateDate, coordinates) {
    try {
        ...
        const [canvasPage] = await newPdfDoc.copyPages(pdfDoc, [0]); // Copy the original PDF page
        newPdfDoc.addPage(canvasPage); // Add the copied page to the new PDF document

        // Add the canvas content as a new page
        const canvasImage = await newPdfDoc.embedPng(canvasAsBuffer);
        const canvasPage = newPdfDoc.addPage([canvas.width, canvas.height]);
        const canvasPageCtx = canvasPage.getContext();
        canvasPageCtx.drawImage(canvasImage, {
            x: 0,
            y: 0,
            width: canvas.width,
